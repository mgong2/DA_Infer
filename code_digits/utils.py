...
def prepare_parser():
    ...
    parser.add_argument(
        '--source_dataset', type=str, default='I128_hdf5',
        help='Which Dataset to train on, out of I128, I256, C10, C100;'
             'Append "_hdf5" to use the hdf5 version for ISLVRC '
             '(default: %(default)s)')
    parser.add_argument(
        '--target_dataset', type=str, default='I128_hdf5',
        help='Which Dataset to train on, out of I128, I256, C10, C100;'
             'Append "_hdf5" to use the hdf5 version for ISLVRC '
             '(default: %(default)s)')
    parser.add_argument(
...
def get_data_loaders(dataset, data_root, augment=False, batch_size=50,
...
def prepare_root(config):
    for key in ['weights_root', 'logs_root', 'samples_root']:
        ...
        if not os.path.exists(config[key]+'/' + config['source_dataset'] + '_' + config['target_dataset']):
            print('Making directory %s for %s...' % (config[key], key))
            os.mkdir(config[key]+'/' + config['source_dataset'] + '_' + config['target_dataset'])
...
def load_C(C_MI, weights_root, experiment_name, strict=True):
    root = '/'.join([weights_root])

    C_MI.classifier.load_state_dict(
        torch.load('%s/%s.pth' % (root, 'c_099')),
        strict=strict)
...
def sample_sheet(G, classes_per_sheet, num_classes,num_domain, samples_per_class, parallel,
    ...
    for k in range(num_domain):
        for i in range(num_classes // classes_per_sheet):
            ...
            yd = torch.ones(y.size(), device='cuda').long()*k
            for j in range(samples_per_class):
                z_ = torch.randn(classes_per_sheet, G.dim_z, device='cuda')
                with torch.no_grad():
                    if parallel:
                        o = nn.parallel.data_parallel(G, (z_[:classes_per_sheet], G.shared(y), G.shared_d(yd)))
                    else:
                        o = G(z_[:classes_per_sheet], G.shared(y), G.shared_d(yd))
            ...
            image_filename = '%s/%s/%d/samples%s%d.jpg' % (samples_root, experiment_name,
                                                           folder_number, 'source' + str(k), i)
            torchvision.utils.save_image(out_ims, image_filename,
                                         nrow=samples_per_class, normalize=True)

def sample_sheet_inter(G, classes_per_sheet, num_classes, num_domain, samples_per_class, parallel,
                 samples_root, experiment_name, folder_number, z_=None):
    # Prepare sample directory
    if not os.path.isdir('%s/%s' % (samples_root, experiment_name)):
        os.mkdir('%s/%s' % (samples_root, experiment_name))
    ...
    for k in range(num_domain):
        for i in range(num_classes // classes_per_sheet):
            ...
            if k<num_domain-1:
                ...
                r = 0
            yd_r = torch.ones(y.size(), device='cuda').long()*r
            for j in range(samples_per_class):
                z_ = torch.randn(classes_per_sheet, G.dim_z, device='cuda')
                with torch.no_grad():
                    if parallel:
                        o = nn.parallel.data_parallel(G, (z_[:classes_per_sheet], G.shared(y), (G.shared_d(yd)+ G.shared_d(yd_r))/2))
                    else:
                        o = G(z_[:classes_per_sheet], G.shared(y), (G.shared_d(yd)+ G.shared_d(yd_r))/2)
            ...
            # The path for the samples
            image_filename = '%s/%s/%d/samples%s%d.jpg' % (samples_root, experiment_name,
                                                           folder_number, 'inter_' + str(k) + '_' + str(r), i)
            torchvision.utils.save_image(out_ims, image_filename,
                                         nrow=samples_per_class, normalize=True)
...
def interp_sheet(G, num_per_sheet, num_midpoints, num_classes, parallel,
    ...
    if fix_yd:  # If fix y, only sample 1 z per row
        yds = sample_1hot(num_per_sheet, 4)
        yds = G.shared(yds).view(num_per_sheet, 1, -1)
        yds = yds.repeat(1, num_midpoints + 2, 1).view(num_per_sheet * (num_midpoints + 2), -1)
    else:
        yds = interp(G.shared_d(sample_1hot(num_per_sheet, 4)).view(num_per_sheet, 1, -1),
                    G.shared_d(sample_1hot(num_per_sheet, 4)).view(num_per_sheet, 1, -1),
                    num_midpoints).view(num_per_sheet * (num_midpoints + 2), -1)
    ...
    with torch.no_grad():
        if parallel:
            out_ims = nn.parallel.data_parallel(G, (zs, ys, yds)).data.cpu()
        else:
            out_ims = G(zs, ys, yds).data.cpu()
    interp_style = '' + ('Z' if not fix_z else '') + ('Y' if not fix_y else '')+ ('Yd' if not fix_yd else '')
...
def name_from_config(config):
    name = '_'.join([
        item for item in [
            config['source_dataset'] + '_' + config['target_dataset']+'/',
            'num_domain: ' + str(config['n_domain']),
            config['loss_type'],
...
def prepare_z_y(G_batch_size, dim_z, nclasses, ndomains, device='cuda',
    ...
    yd_ = Distribution(torch.zeros(G_batch_size, requires_grad=False))
    yd_.init_distribution('categorical', num_categories=ndomains)
    yd_ = yd_.to(device, torch.int64)
    return z_, y_, yd_
